...
<!--
<ul>
	<li>Перепись кода на ООП, чистка и профессиональное оформление кода</li>
	...
	<li>Анализ MySQL запросов (лог медленных запросов) и их оптимизация (если возможно)</li>
	<li>Повышение безопасности</li>
	<li>Вывод количества реализованных заказов по месяцам по каждому модельеру в разделе статистики</li>
	<li>php-акселлераторы ?</li>
</ul>
-->
...
<table style="line-height:1.6; padding:0 2% 1% 1%; text-align: justify;">
	...
	<!-- <tr><td style="vertical-align:text-top; color:grey"><i>0.3</i></td><td style="color:grey;padding-bottom: 1%;">Новый виток развития. Каких-либо 
		визуальных изменений практически нет, все изменения и доработки "под капотом" сайта. Текущая ветка имеет чисто технический характер.
		В первую очередь уделено внимание безопасности сайта, его стабильности и оптимизации, с целью повышения производительности.
		Внедрено кэширования данных, при помощи <a href="http://redis.io/"><i>Redis</i></a>. Проанализированы и оптимизированы все MySQL-запросы, 
		добавлены недостающие индексы к таблицам. Более тесная и полная интеграция с фреймворком Yii. 
		Подключены php-акселлераторы. Полная переработка и оптимизация кода.</td></tr> -->

	<tr><td style="vertical-align:text-top;"><i>0.2.4</i></td><td>Небольшая переработка структуры БД: добавленина возможность
		заносить свои материалы (в разделе администрирования). Вывод в разделе статистики информации о количестве реализованных заказов по месяцам 
		по каждому модельеру. Для того чтобы увидеть эту информацию, необходимо кликнуть на название того месяца, подробности которого, вас интересуют. 
		Небольшой багфикс.</td></tr>

	<tr><td style="vertical-align:text-top;"><i>0.2.3</i></td><td>Корректирующий релиз.
		Исправлена ошибка отображения графиков в разделе статистики при отсутствии записей в базе. Корректное отображение графиков, в тех случаях, когда
		в какой-либо день не было сделано никаких заказов. После создания нового заказа очищаются все поля формы. Более плавная анимация появления
		и исчезновения сообщений об ошибках. Некоторые наработки, связанные с повышением безопасности и производительности (за счет сжатия страниц).
		В раздел статистики добавлено два графика. Один показывает количество заказов в день, сделанные каждым модельером, другой - процент реализованных
		заказов по модельерам. Графики, кроме последнего, можно масштабировать. Поиск стал более точным, исправлен баг поиска заказа по комментарию.
		На странице редактирования заказа доступна кнопка для удаления текущего заказа. Небольшая доработка дизайна: немного видоизменена страница
		с заказами, несколько переработана выдача поисковых результатов, ссылки сортировки заказов (добавлены новые) перемещены в заголовок таблицы,
		а также множество других мелких доработок.</td></tr>

	<tr><td style="vertical-align:text-top"><i>0.2.2</i></td><td>Добавлена возможность сохранять заказы
		в Excel за выбранный промежуток времени (реализована с помощью <a href="http://phpexcel.codeplex.com/"><i>PHPExcel</i></a>) и графический
		раздел статистики (реальзован с помощью <a href="http://www.jqplot.com/"><i>jqPlot</i></a>).
	...
	<tr class="infoMessage"><td style="vertical-align:text-top; padding: 1% 0;"><i><b>Сообщение:</b></i></td><td style="padding: 1% 0;"><b>
		Состоялся <u>релиз</u> приложения для Android по работе с базой данных - 
		<a href="<?php echo Yii::app()->request->baseUrl; ?>/assets/OrthopedicDB.apk">OrthopedicDB</a>(текущая версия 1.0).<b></td></tr>

	<tr><td style="vertical-align:text-top;"><i>0.2</i></td><td>Вновь проведена серьезная переработка сайта - сайт переписан под
		<a href="http://www.yiiframework.com/"><i>Yii</i></a>, что делает его модульным и легко расширяемым. Изменено хранение информации о модели в БД:
	...
	<tr class="infoMessage"><td style="vertical-align:text-top; padding: 1% 0;"><i><b>Сообщение:</b></i></td><td style="padding: 1% 0;"><b>
		Доступна бета-версия приложения для Android по работе с базой данных - <a href="">OrthopedicDB</a> (текущая версия 0.9.1).<b></td></tr>

	<tr><td style="vertical-align:text-top"><i>0.1.6.1</i></td><td>Добавлен плагин jQuery FancyBox. Доработано автозаполнение
	...
	<tr><td style="vertical-align:text-top"><i>0.1.5.6</i></td><td> Проверка заполнения форм до отправки. Исправлено автозаполнение
		...
		<a href="http://jquery.com/"><i>jQuery</i></a>!</td></tr>
