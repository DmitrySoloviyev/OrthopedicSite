...
<div id="tabs" class="effect8" style="width:681px;">
    ...
    <div class="form">
		...
		<legend>Новый сотрудник</legend>
			<div>
				<div class="row">
					<?php
					echo $form->TextField($employeesModel,'EmployeeSN', array('autocomplete'=>'Off', 'maxlength'=>'29', 'placeholder'=>'Фамилия'));
					echo $form->error($employeesModel,'EmployeeSN');?>
				</div>
				<div class="row">
					<?php
					echo $form->TextField($employeesModel,'EmployeeFN', array('autocomplete'=>'Off', 'maxlength'=>'29', 'placeholder'=>'Имя'));
					echo $form->error($employeesModel,'EmployeeFN');?>
				</div>
				<div class="row">
					<?php
					echo $form->TextField($employeesModel,'EmployeeP', array('autocomplete'=>'Off', 'maxlength'=>'29', 'placeholder'=>'Отчество'));
  ...
  <div id="tabs_2">
    	<?php
    		$result = Employees::model()->findAllBySql("SELECT CONCAT_WS(' ', EmployeeSN, EmployeeFN, EmployeeP) AS FIO FROM Employees WHERE STATUS = 'Работает'");
  ...
  <div id="tabs_3">
    	<?php
    		$result = Employees::model()->findAllBySql("SELECT CONCAT_WS(' ', EmployeeSN, EmployeeFN, EmployeeP) AS FIO FROM Employees WHERE STATUS = 'Уволен'");
    		foreach ($result as $key => $value) {
    			echo "<div>".$value->FIO."</div>";
    		}
    	?>
...
<table>
	...
	<tr class="infoMessage"><td style="vertical-align:text-top"><i><b>Сообщение:</b></i></td><td>
		<b>Доступна бета-версия приложения для Android по работе с базой данных - <a href="<?php echo Yii::app()->request->baseUrl; ?>
		/assets/OrthopedicDB.apk">OrthopedicDB</a> (текущая версия 0.9.1).<b></td></tr>
	...
	<tr><td style="vertical-align:text-top"><i>0.1.5.6</i></td><td> Проверка заполнения форм до отправки. Исправлено автозаполнение
		комментариев, внесено множество некоторых исправлений. Добавлена поддержка возможностей JavaScript и jQuery!</td></tr>
