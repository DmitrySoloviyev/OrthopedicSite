...
<table style="line-height:1.6; padding:0 2% 1% 1%; text-align: justify;">
    ...
    <tr>
        ...
        <td style="padding-bottom: 1%;"><i>Продожена работа по оптимизации производительности</i>: проведено эталонное тестирование
        и профилирование с целью верной расстановки индексов в БД. Нагрузка эмулировалась при помощи
        <a href="http://httpd.apache.org/docs/2.2/programs/ab.html">Apache Benchmark tool (ab)</a>.<br><i>Продожена работа по
        повышению безопасности</i>: Разграничение прав доступа на основе системы доступа на основе ролей
        ...
        администратор имеет возможность назначать роли пользователям.<br><i>Остальное:</i>
        В раздел статистики добавлены 3 графика, аналогичные существующим, но по моделям. Ресайз загружаемых
        ...
        выводится сообщение с предложением ее создать. Повышена безопасность сайта.
        <p class="release_date">(xx.xx.2015)</p>
        </td>
    </tr>
    -->
    ...
    <tr>
        ...
        <td style="padding-bottom: 5%;">Yii обновлен до версии
            ...
            пункт
            выбора модельера из выпадающего списка). По аналогии с заказами, у моделей появился атрибут автор
            (заполняется
            автоматически). Повышена безопасность сайта. Комментарии к заказам и моделям и поле описание у моделей
            теперь
            являются полнотекстовыми и больше не ограниченны по длине. Добавлен специальный виджет для форматирования
            текста при заполнении комментариев. Серьезно изменена и переписана визуальная основа сайта: внешний вид
            базируется на
