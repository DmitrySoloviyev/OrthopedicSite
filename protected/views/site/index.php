...
<!--    <tr>
    ...
    <tr>
        ...
        <td style="color:grey;padding-bottom: 2%;">Новый виток развития. Каких-либо визуальных изменений практически нет,
            все изменения и доработки "под капотом" сайта. Текущая ветка имеет чисто технический характер. В первую
            очередь уделено внимание безопасности сайта, его стабильности и оптимизации, с целью повышения производительности.
            Внедрено кэширования данных (реализовано при помощи <a href="http://redis.io/">Redis</a>).
            Проведено профилирование запросов: проанализированы и оптимизированы все MySQL-запросы, добавлены недостающие
            индексы к таблицам. Несколько переработана структура БД, а ее создание вынесено в миграцию, что
            позволит в будущем производить безболезненные изменения в ее структуре, если это потребуется.
            Включено кэширование схем БД для улучшения производительности. Более тесная и полная интеграция с фреймворком
            Yii. Автоматическое развертывание веб-приложения при помощи <a href="http://www.vagrantup.com/">Vagrant</a>.
            Подключены php-акселлераторы, php-fpm. Для работы сайта теперь потребуется версия PHP не ниже 5.4. Полная
            переработка, оптимизация и профессиональное оформление кода. Блочная верстка страниц. Новое окно с
            регистрацией и просмотром модельеров на главной странице, предпросмотр загружаемого изображения модели,
    ...
    <tr>
        ...
        <td>Добавлена возможность сохранять заказы
            в Excel за выбранный промежуток времени (реализована с помощью <a href="http://phpexcel.codeplex.com/">PHPExcel</a>)
    ...
    <tr class="infoMessage">
        ...
        <td style="padding: 1% 0;"><b> Состоялся <u>релиз</u> приложения для Android по работе с базой данных -
            <a href="<?php echo Yii::app()->request->baseUrl; ?>/assets/OrthopedicDB.apk"><i>OrthopedicDB</i></a>(текущая
            версия 1.0).<b></td>
    </tr>
