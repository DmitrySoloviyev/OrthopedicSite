...
$result = Yii::app()->db
	->createCommand("SELECT COUNT(DAY(Date)) AS COUNT, DAYOFMONTH(Date) AS DAY, MONTHNAME(Date) AS MONTH, DATE(Date) AS DAYDATE FROM Orders 
 ...
 * ВТОРОЙ ГРАФИК
 */
$result2 = Yii::app()->db->createCommand("SELECT COUNT(DAY(Date)) AS COUNT, DAYOFMONTH(Date) AS DAY, MONTHNAME(Date) AS MONTH, DATE(Date) AS DAYDATE, EmployeeID,
 ...
 * ТРЕТИЙ ГРАФИК
 */
$result3 = Yii::app()->db->createCommand("SELECT COUNT(DAY(Date)) AS COUNT, CONCAT(EmployeeSN, ' ', LEFT(EmployeeFN, 1), '.', LEFT(EmployeeP, 1), '.') as FIO  
		FROM Orders INNER JOIN Employees USING(EmployeeID) 
